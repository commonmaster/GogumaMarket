			...
			<div class="content_div">
				<!--  여기부터 작성하시면 될듯 -->
					<table>
						...
						%>
							<button class="product-btn" onclick="openReply()">구매요청</button>
						<%} %>
						<% if(loginMno != null){
							if(loginMno == product.getUser().getMno()){ %>
						
							<button class="product-btn" onclick="openReply()">구매메세지</button>
						<%}}%>		
						<%
							if (isWriter != null && isWriter == true) {
						%>
						...
						<button class="product-btn" onclick="location.href='/products?pageno=${pageno}&search=${search}'">목록</button></td>
						</tr>
					</table>
						...
						<div class="my-3 p-3 bg-white rounded shadow-sm" id="reply_input_div" style="padding-top: 10px">
							...
							<div>
								<div class="col-sm-10" id="col-sm-10_add">
									<textarea class="form-control" id="contents" rows="3" maxlength="100" placeholder="변경할 메시지를 입력하세요(100자)"></textarea>
...
$(document).ready(function () {
		...
		var saler = <%=product.getUser().getMno() %>;
		if(loginMno != null){
			showReplyList();
		}
		...
		// 접속자에 따라 댓글 placeholder 변경
		if(loginMno == saler){
			$("#contents").attr("placeholder", "구매자에게 보낼 메세지를 작성하세요(100자)");
		}
		else{
			$("#contents").attr("placeholder", "구매할 분의 연락처 및 메세지를 남겨주세요(100자)");
		}
...
//수정폼
function editReply(rno, pmno, id, mno, name, contents, cdate){
		var htmls = "";
